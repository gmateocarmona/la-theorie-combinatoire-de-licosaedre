...
\foreach \k in {1,...,5}{
    \coordinate (s\k) at ({90+72*(\k-1)}:\R);
}

\coordinate (s0) at (s5);
\coordinate (c) at (0,0);

\draw (s1)--(s2) node[midway,above right] {$a_4$};
\draw (s2)--(s3) node[midway,right] {$a_8$};
\draw (s3)--(s4) node[midway,below right] {$a_1$};
\draw (s4)--(s0) node[midway,below left] {$a_2$};
\draw (s0)--(s1) node[midway,left] {$a_3$};

\draw (c)--(s1) node[midway,left] {$h_4$};
\draw (c)--(s2);
\draw (c)--(s3) node[midway,right] {$h_1$};
\draw (c)--(s4) node[midway,below] {$h_2$};
\draw (c)--(s0) node[midway,left] {$h_3$};
...
À cause de la propriété de la connexité, les seuls sommets sont~: $\{ s, s_0, \dots, s_4, t_0, \dots, t_4, \infty \}$

%%

Alors l'unique isomorphisme $g \in \operatorname{Isom}(\Pi, \Pi')$ qui transforme un repère $r \in \operatorname{Rep}(\Pi)$ en $r' \in \operatorname{Rep}(\Pi')$ est l'isomorphisme qui est défini par~:
\[
g : \Pi \isommap \Pi'
\]
\[
...
\vskip .3cm
{
Proposition I.4. --- \it Soit $\Pi = (S, A, F, R, R', R'')$ un icosaèdre alors
\begin{itemize}
    \item[i.)] $\operatorname{card} S = 12$
    \item[ii.)] $\operatorname{card} A = 30$
    \item[iii.)] $\operatorname{card} F = 20$
    \item[iv.)] $\operatorname{card} \operatorname{Rep}(\Pi) = \operatorname{card } \operatorname{Aut}(\Pi) = 120$
\end{itemize} 
}
\vskip .3cm
Démonstration. --- \begin{itemize}
    \item[i.)] trivial
    \item[ii.)] $\operatorname{card}(A(s)) = 5$ et $\operatorname{card} S(a) = 2$
    \item[iii.)] $\operatorname{card} F(a) = 2$ et $\operatorname{card} A(f) = 3$
    \item[iv.)] $\Pi$ est régulier $\Rightarrow \operatorname{card} \operatorname{Rep}(\Pi) = \operatorname{card} \operatorname{Aut}(\Pi)$
\end{itemize}
Grâce à la bijection canonique
\[
\{(s, s', s'') \in S \times S \times S \mid \{s, s', s''\} \in F\} \xrightarrow{\sim} \operatorname{Rep} \Pi
\]
\[
(s, s', s'') \longmapsto (s, \{s, s'\}, \{s, s', s''\})
\]
$\operatorname{card} \operatorname{Rep}(\Pi)$ est égal à $6$ fois $\operatorname{card} F$ \\
$\Rightarrow \operatorname{card} \operatorname{Rep}(\Pi) = 6 \cdot 20 = 120$

%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{II. Distance combinatoire}\label{sec:2}%
\addcontentsline{toc}{section}{II. Distance combinatoire}

Soit $\Pi = (S, A, F, R, R', R'')$ un polyhedre combinatoire connexe. On a les applications
$$
\varphi: A \hookrightarrow \mathfrak{P}_2(S)
$$
$$
\psi: A \hookrightarrow \mathfrak{P}_2(F)
$$
qui sont injectifs. 
